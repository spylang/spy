...
`EMSCRIPTEN_IMPORT` to declare that a function is defined as an import. The
callback walks the `env` imports and replaces each stub with the corresponding
`env_*` method found on the host modules.
