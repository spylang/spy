...
class W_SimdType(W_Type):
    ...
    lanes), and is defined from the ``W_Simd`` value pyclass â which is what
...
def _lane_py(w_lane: W_Object, w_dtype: W_Type) -> Any:
    """
    Unwrap a SIMD lane W_Object to a plain Python value for interp
    arithmetic.  Floats -> python float, ints -> python int (signedness
    preserved via ``FixedInt.__int__``).  Re-wrapping with the lane ctor
    re-applies the width, and hence the C wraparound/narrowing semantics.
    """
...
def _get_or_make_simd_select(
    ...
    *simple* OpSpec (caching-safe).  The C backend reads the mask/operand C
    ...
    def w_impl(
        ...
        # *canonical* masks (comparison results, lanes 0 / -1), which is the
        # v1 contract.  Arbitrary integer masks are accepted at the type level
