...
def w_CONVERT(vm: 'SPyVM', w_exp: W_Type, wm_x: W_MetaArg) -> W_Func:
    """
    ...
    """
    w_opspec = get_opspec(vm, w_exp, wm_x)
    ...
    got = wm_x.w_static_T.fqn.human_name
    exp = w_exp.fqn.human_name
    err.add('error', f'expected `{exp}`, got `{got}`', loc=wm_x.loc)
    raise err


def get_opspec(vm: 'SPyVM', w_exp: W_Type, wm_x: W_MetaArg) -> W_OpSpec:
    ...
    if w_conv_to := w_got.lookup_func('__convert_to__'):
        wm_exp = W_MetaArg.from_w_obj(vm, w_exp)
        w_opspec = vm.fast_metacall(w_conv_to, [wm_exp, wm_x])
        return w_opspec

    elif w_conv_from := w_exp.lookup_func('__convert_from__'):
        wm_got = W_MetaArg.from_w_obj(vm, w_got)
        w_opspec = vm.fast_metacall(w_conv_from, [wm_got, wm_x])
        return w_opspec
...
def CONVERT_maybe(
        vm: 'SPyVM', w_exp: W_Type, wm_x: W_MetaArg,
) -> Optional[W_Func]:
    """
    ...
    if vm.issubclass(w_got, w_exp):
        # nothing to do
        return None
    return vm.fast_call(OP.w_CONVERT, [w_exp, wm_x])  # type: ignore
