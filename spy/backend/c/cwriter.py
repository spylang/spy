...
class CFuncWriter:
    ...
    def _call_codegen_args(self, call: ast.Call, irtag: IRTag) -> list[ast.Expr]:
        # ptr.getitem/ptr.store carry an extra trailing LocConst which is not part of the C ABI.
    ...
    def collect_call_temps(self) -> None:
        for node in self.w_func.funcdef.walk():
            ...
            if irtag.tag in ("struct.getfield", "ptr.getfield"):
                continue
            args_for_temps = self._call_codegen_args(call, irtag)
            needs_temps = any(
                self._arg_contains_assignexpr(arg) or self._arg_contains_call(arg)
                for arg in args_for_temps
            )
            if not needs_temps:
                continue
            ...
            if not isinstance(w_obj, W_Func):
                continue
            c_types = self._call_arg_types_for_temps(w_obj, args_for_temps)
            temp_names = [self._new_temp_var(c_type) for c_type in c_types]
    ...
    def fmt_expr_Call(self, call: ast.Call) -> C.Expr:
        ...
        if op := self.FQN2BinOp.get(fqn):
            ...
            return self.fmt_generic_call(fqn, call, args=args)
    ...
    def _fmt_call_arg(
        self, arg: ast.Expr, temp_name: str | None
    ) -> tuple[list[C.Expr], C.Expr]:
        assignexpr = self._assignexpr_parts(arg)
        if temp_name is None:
            if assignexpr is None:
                return [], self.fmt_expr(arg)
            target, value_expr = assignexpr
            return [self._fmt_assignexpr(target, value_expr)], C.Literal(
                f"{C_Ident(target)}"
            )

        temp_ident = C.Literal(f"{C_Ident(temp_name)}")
        if assignexpr is None:
            return [C.BinOp("=", temp_ident, self.fmt_expr(arg))], temp_ident
        target, value_expr = assignexpr
        return [
            self._fmt_assignexpr(target, value_expr),
            C.BinOp("=", temp_ident, C.Literal(f"{C_Ident(target)}")),
        ], temp_ident

    def _fmt_call_args(
        self, call: ast.Call, args: list[ast.Expr] | None = None
        ...
        if temp_names is not None:
            ...
            for arg, temp_name in zip(args, temp_names, strict=True):
                arg_prelude, c_arg = self._fmt_call_arg(arg, temp_name)
                prelude_temps.extend(arg_prelude)
                c_args_temps.append(c_arg)
            return c_args_temps, prelude_temps

        has_assignexpr = any(self._assignexpr_parts(arg) is not None for arg in args)
        ...
        prelude: list[C.Expr] = []
        for arg in args:
            arg_prelude, c_arg = self._fmt_call_arg(arg, None)
    ...
    def fmt_generic_call(
        self, fqn: FQN, call: ast.Call, *, args: list[ast.Expr] | None = None
    ) -> C.Expr:
        # default case: call a function with the corresponding name
        self.ctx.add_include_maybe(fqn)
        c_name = fqn.c_name
        c_args, prelude = self._fmt_call_args(call, args=args)
